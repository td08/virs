...
namespace Virs_Client_Form
{
    class WavBuilder
    {
        public static void generateWav(string path, int[] rawData)
        {
            string writeFilePath = Path.Combine(path, "steth.wav");
            ...
            using (FileStream fs = new FileStream(writeFilePath, FileMode.Create))
