...
namespace AES_Client
{
    class AesServiceProvider
    {
        ...
        public byte[] localPubKeyBlob { get; private set; }  // local public key blob buffer
        public byte[] remotePubKeyBlob { get; set; }       // remote public key blob buffer
        public byte[] symmetricKey { get; private set; }          // symmetric key buffer
        ...
        // returns symKey containing the decrypted 256-bit symmetric key to be used for all further encrypted communications
        public byte[] getSymmetricKey(byte[] keyExchangeInfo)
        {
            byte[] symKey = null;
            ...
            try
            {
                ...
                using (CngKey remotePubKey = CngKey.Import(remotePubKeyBlob, CngKeyBlobFormat.EccPublicBlob))
                {
                    ...
                    {
                        cs.Write(keyExchangeInfo, 0, keyExchangeInfo.Length - nBytes);
                        cs.FlushFinalBlock();   // ensures the ms buffer is updated with all data from the cryptostream
                        symKey = ms.ToArray();
                    }
                }
